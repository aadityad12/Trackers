...
- **Verified** by `assembleDebug` + `testDebugUnitTest` + `lintDebug` (0 errors) only — **no on-device/emulator smoke test** (another process owns the device). The `MIGRATION_13_14` PK-change dance in particular is only schema-validated against the exported `14.json`, not exercised on a real upgrade.
