...
/**
 * Overview's stat cards format study/screen time via [formatTimeCompact] and currency via
...
class OverviewFormattingTest {
    ...
    fun `study and screen time under an hour show only minutes`() {
        assertEquals("45m", formatTimeCompact(45 * 60L))
    }

    @Test
    fun `study and screen time over an hour show hours and minutes`() {
        assertEquals("1h 30m", formatTimeCompact(90 * 60L))
    }

    @Test
    fun `zero minutes formats as 0m, not blank`() {
        assertEquals("0m", formatTimeCompact(0L))
    }
